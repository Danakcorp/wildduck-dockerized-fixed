...
Additionally, the provided setup currently uses a very basic setup where all the services are ran on the same domain. Ideally you'd want to run outbound SMTP (port 465 SMTPS / port 587 STARTTLS), imap, pop3, inbound smtp (port 25 Haraka), on different domains and have separate certs for them (will be handled by Traefik automatically, except for Haraka and ZoneMTA STARTTLS which use files from `./certs`). For database and redis sharding refer to Wildduck and Zone-MTA documentation.  
